"""Build a labeled, deduplicated classification dataset for CHEMBL379 IC50 data.

This script applies the thesis curation policy recorded in Memory.md:
- filter to CHEMBL379 IC50 measurements in nM
...
RAW_DATASET_PATH = REPO_ROOT / "datasets" / "CHEMBL379_IC50_AllDesc.csv"
...
def _compute_threshold_nm(rows: list[AssayRow]) -> float:
    """Compute the classification threshold from exact IC50 rows only.

    Args:
        rows: Scope-filtered assay rows.

    Returns:
        The median IC50 threshold in nM.

    Raises:
        ValueError: If no exact IC50 rows are available.
    """

    exact_values_nm = [
        row.standard_value_nm for row in rows if row.standard_relation == "'='"
    ]
    if not exact_values_nm:
        raise ValueError("No exact IC50 rows were found for threshold computation.")
...
def _resolve_activity_label(row: AssayRow, threshold_nm: float) -> LabeledRow | None:
    ...
    if relation == "'>'" and value_nm >= threshold_nm:
